...
Rcs = [0.0946, 0.0462, 0.4250]
Ban = [550, 650, 780]
Rct = [0.1303, 0.0722, 0.3665]
Rgs = [0.1268, 0.1742, 0.2292]
Rncs = [0.001, 0.001, 0.001]
...
for LAIa in  np.arange(LAI_min, LAI_max + LAI_interval, LAI_interval):
    if LAIa > LAI_max:
            ...
            for ss_j in range(ss_number):
                for i in range(v_number):
                    for j in range(s_number):
                        ...
                        ksss = hsss / 2 / Rsss
                        lamtai = min(1, 3 * np.sqrt((np.cos(m_v / 180 * np.pi) ** 2) + (ksss ** 2) * (np.sin(m_v / 180 * np.pi) ** 2)) / 8 / G / Rsss / ksss / usss)
                        print(lamtai)
                        '''
                        A1 = a1 / np.sin(vv_ss / 180 * np.pi)
                        A2 = a2 / np.sin(vv_ss / 180 * np.pi)
                        A = A1 + A2
                        # 计算临界角c
                        c = np.arctan(A / h) * 180 / np.pi
                        # 计算u叶面积体密度
                        u = LAIa / h
                        if m_v >= c:
                            lamtai = 1
                        else:
                            p = ((A1 - h * np.tan(m_v / 180 * np.pi) - 2 * np.sin(m_v / 180 * np.pi) / G / u)
                                * np.exp(-G * u * h / np.cos(m_v / 180 * np.pi)) +
                                (A2 - h * np.tan(m_v / 180 * np.pi) + 2 * np.sin(m_v / 180 * np.pi) / G / u)) / A
                            print(p)
                            ci = -np.cos(m_v / 180 * np.pi) * np.log(p) / G / LAIa
                            lamtai = ci
                        print(lamtai)
                        '''
                        ...
                        for ii in range(Bands):
                            for jj in range(s_number):
                                ...
                                m1 = 0.5 * (1 - belta[jj][ii]) * i0 * math.pow((w1[ii]), 2) * p * (1 - p) / (1 - w1[ii] * p)
                                m1 = round(m1, 5)

                                #S1 = np.exp(-0.5867 * LAIa)
                                S1 = 0.46279 * np.exp(-0.46779 * lamtai * LAIa) + 0.01127
                                m2 = belta[jj][ii] * (1 - S1) * ia / 2 * math.pow((w1[ii]), 2) * p * (1 - p) / (1 - w1[ii] * p)
                                m2 = round(m2, 5)

                                Sbs = i0 * math.pow(w1[ii], 2) * m1 * (1 - m1) / (1 - w1[ii] * m1)
                                rc = (0.5 / ia) * Sbs
                                m3 = (1 - belta[jj][ii]) * (1 - i0) * Rgs[ii] * ia * rc / (1 - Rgs[ii] * ia * rc) * (1 + Rgs[ii] * (1 - ia))
                                m3 = round(m3, 5)

                                m4 = (1 - belta[jj][ii]) * i0 * rc * Rgs[ii] / (1 - Rgs[ii] * ia * rc) * (1 - ia + ia * rc)
                                m4 = round(m4, 5)

                                m5 = belta[jj][ii] * S1 * (1 - ia) * Rgs[ii] * ia * rc / (1 - Rgs[ii] * ia * rc) * (1 + Rgs[ii] * (1 - ia))
                                m5 = round(m5, 5)

                                m6 = belta[jj][ii] * (1 - S1) * Rgs[ii] * ia * rc / (1 - Rgs[ii] * ia * rc) * (1 - ia + ia * rc)
                                m6 = round(m6, 5)
                        ...
                        ax.grid(True)
                        plt.title("LAI：" + str(LAIa) + "  角度：" + str(m_v) + " " + str(vv_need) + " " + str(m_s) + " " + str(ss_need))

                        plt.legend([e1, e2], ['入射', '观测'], loc='upper right')

                        #plt.show()
                        plt.savefig("./img/" + "LAI：" + str(LAIa) + "  角度：" + str(m_v) + " " + str(vv_need) + " " + str(m_s) + " " + str(ss_need) + ".png")
                        ...
                        (x * sympy.cos(vv_ss / 180 * np.pi) + y * sympy.sin(vv_ss / 180 * np.pi) - cv)**2 / rlv**2 + (y * sympy.cos(vv_ss / 180 * np.pi) - x * sympy.sin(vv_ss / 180 * np.pi))**2 / r**2 - 1], 
                        [x,y])
                        ...
                        if len(solved_value) == 0:
                            if math.pow((vtoix - ci), 2) / math.pow(rli, 2) + math.pow(vtoiy, 2) / math.pow(r, 2) - 1 < 0 or math.pow((itovx - cv), 2) / math.pow(rlv, 2) + math.pow(itovy, 2) / math.pow(r, 2) - 1 < 0:
                                ...
                                lc = 0
                                #print('情况2')
                        elif (str(type(solved_value[0][0].evalf()))=='<class \'sympy.core.numbers.Float\'>')==False:
                            lc = lv
                            #print('情况3')
                        else:
                            #print('情况4')
                            ...
                            '''
                            # 求i椭圆扇形面积
                            lci_sector = 0.5 * rli * r * np.arccos((r * r * (x1 - ci) * (x2 - ci) + rli * rli * y1 * y2) / (r * r * rli * rli))
                            ...
                            lcv_sector = 0.5 * rlv * r * np.arccos((r * r * (x1tov - cv) * (x2tov - cv) + rlv * rlv * y1tov * y2tov) / (r * r * rlv * rlv))
                            ...
                            s_helenv = np.sqrt(p_helenv * (p_helenv - alongv) * (p_helenv - blongv) * (p_helenv - clongv))
                        ...
                        for k in range(Bands):
                            k_ratio[count][9 + k] = (1 - belta[tem1][k]) * (kcl * Rcs[k] + kg * Rgs[k]) + \
                                                    (1 - S) * belta[tem1][k]* kt * Rcs[k] + S * belta[tem1][k] * kz * Rgs[k] + \
        ...
        writer = pd.ExcelWriter(str(round(LAIa, 2)) + '.xlsx')
